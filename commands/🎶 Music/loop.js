    ...
    try {
      ...
      if (!args[0])
        return message.channel.send(new MessageEmbed()
          ...
          .setFooter(es.footertext, es.footericon)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:no:921989165242003476> Please add your method!`)
      ...
      if (args[0].toLowerCase() === `song` || args[0].toLowerCase() === `track` || args[0].toLowerCase() === `s` || args[0].toLowerCase() === `t`) {
        ...
        let embed = new MessageEmbed()
          .setTitle(`<a:yes2:921988805442043925> ${emoji.msg.repeat_mode} Changed Track loop to: ${player.trackRepeat ? `${emoji.msg.disabled} disabled` : `${emoji.msg.enabled} active`}`)
      ...
      //if input is queue
        ...
        //Create the Embed
        let embed = new MessageEmbed()
          .setTitle(`<a:yes2:921988805442043925> ${emoji.msg.repeat_mode} Changed Queue loop to: ${player.queueRepeat ? `${emoji.msg.disabled} disabled` : `${emoji.msg.enabled} active`}`)
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          .setFooter(es.footertext, es.footericon)
