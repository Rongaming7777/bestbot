  ...
  run: async (client, message, args, cmduser, text, prefix, player) => {
    ...
    try{
      ...
      if (!args[0]) return message.channel.send(new MessageEmbed()
        ...
        .setTitle(`<a:no:921989165242003476> What should be the search Type`)
      ...
      if (!args[1]) return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor).setFooter(es.footertext, es.footericon)
        .setTitle(`<a:no:921989165242003476> What should I search for?`)
        .setDescription(`Useage: \`${prefix}searchradio <TYPE> <Seach Query>\`\nValid Types: \`country\`, \`city\`, \`name\`, \`genre\`\nExample: \`${prefix}searchradio tag jazz\`\nExample: \`${prefix}searchradio state Austria\``)
      );
      ...
      RadioBrowser.getStations(filter)
        .then(async data => {
          ...
          try {
            ...
            return message.channel.send(new MessageEmbed()
              .setTitle("<a:no:921989165242003476> You didn't provide a selection")
              .setColor(es.wrongcolor)
              .setFooter(es.footertext, es.footericon)
            );
          }
          ...
          if (first.toLowerCase() === 'end') {
            ...
            return message.channel.send(new MessageEmbed()
              .setColor(es.wrongcolor)
              .setFooter(es.footertext, es.footericon)
              .setTitle('<a:no:921989165242003476> Cancelled selection.')
            );
          }
          ...
          if (isNaN(index))
            return message.channel.send(new MessageEmbed()
              .setColor(es.wrongcolor)
              .setFooter(es.footertext, es.footericon)
              .setTitle(`<a:no:921989165242003476> The number you provided is not a Number within (1-${counter}).`)
            );
          if (index < 0 || index > counter - 1)
            return message.channel.send(new MessageEmbed()
              .setColor(es.wrongcolor)
              .setFooter(es.footertext, es.footericon)
              .setTitle(`<a:no:921989165242003476> The number you provided too small or too big (1-${counter}).`)
            );
