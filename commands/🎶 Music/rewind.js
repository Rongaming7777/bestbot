  ...
  run: async (client, message, args, cmduser, text, prefix, player) => {
    ...
    try{
      ...
      if (!args[0])
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setTitle(`<a:no:921989165242003476> You may rewind for \`1\` - \`${player.queue.current.duration}\``)
        );
      ...
      return message.channel.send(new MessageEmbed()
        .setTitle(`<a:yes2:921988805442043925> ${emoji.msg.rewind} Rewinded the song for \`${args[0]} Seconds\` to: ${format(Number(player.position))}`)
