    ...
    try{
      ...
      if (!role)
        return message.channel.send(new MessageEmbed()
          ...
          .setTitle(`<a:no:921989165242003476> Please add a Role via ping, @role!`)
        );
      ...
      try {
        ...
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:no:921989165242003476> It seems that the Role does not exist in this Server!`)
        );
      }
      ...
      if(client.settings.get(message.guild.id,`djroles`).includes(role.id))
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:no:921989165242003476> This Role is alerady in the List!`)
        );
      ...
      return message.channel.send(new MessageEmbed()
        ...
        .setFooter(es.footertext, es.footericon)
        .setTitle(`<a:yes2:921988805442043925> Added the DJ ROLE \`${role.name}\``)
