    ...
    try{
      ...
      if (message.member.guild.owner.id !== message.author.id)
        return message.channel.send(new MessageEmbed()
          ...
          .setTitle(`<a:no:921989165242003476> You don\'t have permission for this Command! *Only the Server-Owner*`)
        );
      //ask for second yes
        ...
        .then(async collected => {
          ...
          if(collected.first().content.toLowerCase() === `yes`)
          {
            ...
            return message.channel.send(new MessageEmbed()
              .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
              .setFooter(es.footertext, es.footericon)
              .setTitle(`<a:yes2:921988805442043925> Resetted everything!`)
          ...
          return message.channel.send(new MessageEmbed()
            .setColor(es.wrongcolor)
            .setFooter(es.footertext, es.footericon)
            .setTitle(`<a:no:921989165242003476> CANCELLED CAUSE NOT THE RIGHT WORD / TIME RAN OUT!`)
          );
        })
      });
    } catch (e) {
