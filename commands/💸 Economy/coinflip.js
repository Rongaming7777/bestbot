    ...
    try {
      ...
      if (!flip || !['heads', 'tails'].includes(flip)) 
        return message.reply(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:no:921989165242003476> Specify the \`flip-result\`, it should be a number between \`heads\`-\`tails\``)
      ...
      if (!amount) 
        return message.reply(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:no:921989165242003476> Specify the **amount of \`Coins 💸\`** you want to gamble!`)
          .setDescription(`Usage: \`${prefix}coinflip <roll-result> <Gamble-Amount>\`\n\n\Example: \`${prefix}coinflip heads 100\``)
        );
      if (data.balance < amount) return message.reply(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(es.footertext, es.footericon)
        .setTitle(`<a:no:921989165242003476> You can't gamble more Money than you have in your **👛 Pocket (\`${data.balance} 💸\`)**`)
      );
      ...
      if (win) {
        ...
        message.channel.send(new MessageEmbed()
          .setTitle(`<a:yes2:921988805442043925> You've won \`${amount} 💸\``)
        ...
        //send the Information Message
        message.channel.send(new MessageEmbed()
          .setTitle(`<a:no:921989165242003476> You've lost \`${amount} 💸\``)
          .setDescription(`**The Coin Flipped: \`${result}\`**\n\n👛 You now have \`${nFormatter(Math.floor(data.balance))} 💸\` in your Pocket`)
