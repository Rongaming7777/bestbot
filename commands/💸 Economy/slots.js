    ...
    try {
      ...
      if (!amount) 
        return message.channel.send(new MessageEmbed()
          ...
          .setTitle(`<a:no:921989165242003476> You didn't add the slotsamount`)
          .setDescription(`Usage: \`${prefix}slots <Amount>\`\n\n\Example: \`${prefix}slots 420\``)
        );
      if (amount > data.balance) 
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor).setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
          .setDescription(`<a:no:921989165242003476> You can't gamble more Money than you have in your **👛 Pocket (\`${data.balance} 💸\`)**`)
        );
