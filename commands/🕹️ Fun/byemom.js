  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    let es = client.settings.get(message.guild.id, "embed")
      ...
      if(!user && args[0] && args[0].length == 18) {
        ...
        if(!user || user == null || !user.id) return message.reply("<a:no:921989165242003476> I failed finding that User...")
      }
      else {
      ...
      if(!text) return tempmsg.edit(tempmsg.embeds[0]
        .setTitle("<a:no:921989165242003476> You did not enter a Valid Text!")
        .setColor("RED")
