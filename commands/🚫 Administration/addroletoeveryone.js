      ...
      return message.channel.send(new Discord.MessageEmbed()
       ...
       if (map.get(message.guild.id))
         return message.channel.send(new MessageEmbed()
           .setColor(es.wrongcolor)
           .setFooter(es.footertext, es.footericon)
           .setTitle(`<a:no:921989165242003476> There is an active \`addroletoeveryone\` Command already executing in this Server!`)
         );
       ...
       if (message.member.roles.highest.position <= role.position)
         return message.channel.send(new MessageEmbed()
           .setColor(es.wrongcolor)
           .setFooter(es.footertext, es.footericon)
           .setTitle(`<a:no:921989165242003476> I cannot give that Role to all Members, because it's higher then your highest ROLE!`)
         );
