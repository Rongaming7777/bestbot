    ...
    try {
      ...
      if (!role || role == null || role == undefined|| role.name == null || role.name == undefined)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:no:921989165242003476> please ping a ROLE!`)
          .setDescription(` Usage: \`${prefix}removerole @USER @ROLE\``)
        );
      if (member.roles.highest.position >= message.member.roles.highest.position)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:no:921989165242003476> I cannot remove that Role from this Member, because he is higher/Equal to your Rang Position!`)
        );
      if(message.member.roles.highest.position <= role.position)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:no:921989165242003476> I cannot remove that Role from this Member, because it's higher then your highest ROLE!`)
        );
      if(!member.roles.cache.some(r=>r.id == (role.id)))
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:no:921989165242003476> I cannot remove that Role from this Member, because he doesn't have it!`)
        );
      var ge = false;
      ...
      message.channel.send(new MessageEmbed()
        ...
        .setFooter(es.footertext, es.footericon)
        .setTitle(`<a:yes2:921988805442043925> removed \`${role.name}\` from \`${member.user.tag}\``)
      );
