    ...
    try {
      ...
      if(member){
        ...
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:no:921989165242003476> You didn't provide a Text`)
        ...
        try{
          ...
          message.channel.send(new MessageEmbed()
            .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
            .setFooter(es.footertext, es.footericon)
            .setTitle(`<a:yes2:921988805442043925> Successfully Dmed ${member.user.username}`)
          )
        }catch{
          message.channel.send(new MessageEmbed()
            .setColor(es.wrongcolor)
            .setFooter(es.footertext, es.footericon)
            .setTitle("<a:no:921989165242003476> Unable to Dm this User, this is probably because he either blocked me or turned his Dms off!")
          )
        }
      }
      else if(role){
        ...
        if (!members || members == null || members.length == null || members.length == 0)
          return message.channel.send(new MessageEmbed()
            .setColor(es.wrongcolor)
            .setFooter(es.footertext, es.footericon)
            .setTitle(`<a:no:921989165242003476> Found no Members!`)
        ...
        await message.channel.send({content: `<@${message.author.id}>`, embed: new MessageEmbed()
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:yes2:921988805442043925> SUCCESS, send a Dm to \`${succeeded.length}\` / \`${failed.length + succeeded.length}\` Members`)
        ...
        .setTitle(`<a:no:921989165242003476> You need to ping a ROLE or a MEMBER`)
