  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      return message.channel.send(new Discord.MessageEmbed()
        ...
        .setTitle("<a:no:921989165242003476> I am missing the permission to `MANAGE ROLES`!")
      )
      ...
      if (!member)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:no:921989165242003476> please ping a USER!`)
      ...
      if (member.roles.highest.position >= message.member.roles.highest.position)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:no:921989165242003476> I cannot mute this Member, because he is higher/Equal to your Rang Position!`)
        );
      ...
      if (!time)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:no:921989165242003476> please add a TIME!`)
          .setDescription(` Usage: \`${prefix}mute @User <Time+Format(e.g: 10m) / perma> [REASON]\`\n\nExample: \`${prefix}mute @User 10m He is doing bad stuff!\``)
        );
      ...
      if (!mutedrole) {
        ...
        mutedrole = await message.guild.roles.create({
          ...
          message.channel.send(new MessageEmbed()
            .setColor(es.wrongcolor)
            .setFooter(es.footertext, es.footericon)
            .setTitle(`<a:no:921989165242003476> I COULD NOT CREATE A ROLE, sorry`)
          );
        });
      }
      if (mutedrole.position > message.guild.me.roles.highest.position)
        return message.channel.send(new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:no:921989165242003476> I cannot access the Role, because it's above me`)
        );
      ...
      if(time.toLowerCase() === "perma"){
        ...
        message.channel.send(new MessageEmbed()
          ...
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:yes2:921988805442043925> \`${member.user.tag}\` got **MUTED** \`for ever\``)
        ...
        member.send(new MessageEmbed()
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:yes2:921988805442043925> \`${message.author.tag}\` muted you \`for ever\``)
          .setDescription(`Reason:\n> ${reason ? `${reason.substr(0, 1800)}` : `NO REASON`}`)
        );
        ...
        message.channel.send(new MessageEmbed()
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:yes2:921988805442043925> \`${member.user.tag}\` got **MUTED** for \`${ms(mutetime, { long: true })}\``)
          .setDescription(`Reason:\n> ${reason ? `${reason.substr(0, 1800)}` : `NO REASON`}`)
        );
        client.stats.push(message.guild.id+message.author.id, new Date().getTime(), "mute"); 
        member.send(new MessageEmbed()
          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
          .setFooter(es.footertext, es.footericon)
          .setTitle(`<a:yes2:921988805442043925> \`${message.author.tag}\` muted you for \`${ms(mutetime, { long: true })}\``)
          .setDescription(`Reason:\n> ${reason ? `${reason.substr(0, 1800)}` : `NO REASON`}`)
        );
        if(client.settings.get(message.guild.id, `adminlog`) != "no"){
          try{
        ...
        setTimeout(() => {
          try {
            message.channel.send(new MessageEmbed()
              .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
              .setFooter(es.footertext, es.footericon)
              .setTitle(`<a:yes2:921988805442043925> \`${member.user.tag}\` got **UNMUTED** after\`${ms(mutetime, { long: true })}\``)
              .setDescription(`Reason:\n> ${reason ? `${reason.substr(0, 1800)}` : `NO REASON`}`)
            );
