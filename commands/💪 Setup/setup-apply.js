      ...
      await tempmsg.awaitReactions(filter, {
        ...
        .then(async collected => {
          ...
          if (reaction.emoji.name === "1️⃣") {
            ...
            if (errored)
              return message.channel.send(new Discord.MessageEmbed().setColor("RED").setFooter(es.footertext, es.footericon).setTitle("<a:no:921989165242003476> ERROR | TIME RAN OUT / INVALID INPUT | cancelled").setDescription("```" + e.message + "```")).then(msg => msg.delete({
                timeout: 7500
              }))
                              ...
                              message.channel.send(embed.setTitle(`Do you want to add a Role, when some1 applies?`).setDescription("Enter `no`, if not\n\nJust ping the Role")).then(msg => {
                                msg.channel.awaitMessages(m => m.author.id === userid, {
                                  ...
                                  if (collected.first().content.toLowerCase() === "no") {
                                    ...
                                    if (!message.guild.me.roles) return message.channel.send(new Discord.MessageEmbed().setFooter(es.footertext, es.footericon).setColor("RED").setDescription("Make sure that the Role is under me!").setAuthor("<a:no:921989165242003476> ERROR | Could not Access the Role", message.author.displayAvatarURL({
                                      dynamic: true
                                    })))
              ...
              await pickmsg.channel.awaitMessages((m) => m.author.id === message.author.id, {
                  ...
                  if (channel) {
                    ...
                    message.channel.send(new Discord.MessageEmbed()
                      .setColor("RED")
                      .setFooter(es.footertext, es.footericon)
                      .setTitle("<a:no:921989165242003476> ERROR | INVALID INPUT | cancelled")
    ...
    `
            ...
            await pickmsg.awaitReactions((reaction, user) => user.id === message.author.id, {
                  ...
                  message.channel.send(new Discord.MessageEmbed().setFooter(es.footertext, es.footericon).setColor(es.color).setThumbnail(es.thumb ? es.footericon : null).setAuthor("What should be the new accept Role, which will be granted when the User got accepted?", message.author.displayAvatarURL({
                    ...
                    msg.channel.awaitMessages(m => m.author.id === message.author.id, {
                      ...
                      if (guildrole.rawPosition <= botrole.rawPosition) {
                        ...
                        return message.channel.send(new Discord.MessageEmbed().setFooter(es.footertext, es.footericon).setColor("RED").setDescription("Make sure that the Role is under me!").setAuthor("<a:no:921989165242003476> ERROR | Could not Access the Role", message.author.displayAvatarURL({
                          dynamic: true
                        })))
                      }
                    }).catch(error => {
                        ...
                        msg.channel.awaitMessages(m => m.author.id === message.author.id, {
                          ...
                          if (collected.first().attachments.size > 0) {
                            if (collected.first().attachments.every(attachIsImage)) {
                              ...
                              return message.channel.send(new Discord.MessageEmbed()
                                .setTitle(`<a:no:921989165242003476> ERROR | Could not your message as a backgroundimage`)
                                .setColor("RED")
                                .setFooter(es.footertext, es.footericon)
                              );
                            }
                ...
                case "emojithree": {
                  ...
                  default:
                    return message.channel.send(new Discord.MessageEmbed().setColor("RED").setTitle("<a:no:921989165242003476> ERROR please enter a valid Option").setDescription(`Valid Options are: \`message\`, \`setimage\`, \`delimage\`, \`setrole\`, \`delrole\`\n\n\nExample usage: \`${prefix}editsetup emojithree message\` --> follow steps / \`${prefix}editsetup emojithree setrole\` --> follow steps`))
                    break;
                  }
                }
                break;
                ...
                case "applychannel":
                  try {
                    ...
                    message.channel.send(new Discord.MessageEmbed()
                      .setColor("RED")
                      .setFooter(es.footertext, es.footericon)
                      .setTitle("<a:no:921989165242003476> ERROR | ERROR")
