		...
		try {
			...
			if (!repo)
				return message.channel.send({embed: new MessageEmbed()
					...
					.setTitle(`<a:no:921989165242003476> You didn't provide a Repository`)
			...
			if (!username || !repository) 
				return message.channel.send({embed: new MessageEmbed()
					.setColor(es.wrongcolor)
					.setFooter(es.footertext, es.footericon)
					.setTitle(`<a:no:921989165242003476> Repository must be in the form \`username/repository\``)
			...
			if (!body) 
				return message.channel.send({embed: new MessageEmbed()
					.setColor(es.wrongcolor)
					.setFooter(es.footertext, es.footericon)
					.setTitle(`<a:no:921989165242003476> Could not fetch that repo`)
