...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    return message.channel.send(new MessageEmbed()
        ...
        .setTitle(`<a:no:921989165242003476> You didn't pinged to whom you want to pay`)
    ...
    if(!payamount)
      return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<a:no:921989165242003476> You didn't remove the payamount`)
        .setDescription(`Usage: \`${prefix}removemoney <@USER> <Amount>\`\n\n\Example: \`${prefix}removemoney <@442355791412854784> 42069\``)
      );
    if(user.bot || topay.bot) return message.reply("<a:no:921989165242003476> **A Discord Bot can not have Economy!**")
    ...
    return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<a:no:921989165242003476> You can't remove a negative Amount of Money or no Money, to ${topay}`)
      );
    ...
    if(payamount > data2.balance)
      return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<a:no:921989165242003476> You can't remove more Money than you he has in his/her **👛 Pocket (\`${data2.balance} 💸\`)**`)
      );
    ...
    return message.reply(new MessageEmbed()
      .setColor(es.color)
      .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
      .setTitle(`<a:yes2:921988805442043925> **You removeed \`${payamount} 💸\` to \`${topay.tag}\`**`)
