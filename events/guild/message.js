...
module.exports = async (client, message) => {
  try {
        ...
        if (!guild_settings.botchannel.includes(message.channel.id) && !message.member.hasPermission("ADMINISTRATOR")) {
          //create the info string
            ...
            return message.channel.send(new Discord.MessageEmbed()
              ...
              .setTitle(`<a:no:921989165242003476> Error | Not in the Bot Chat!`)
    ...
    if (command && !customcmd){
        if(command.length == 0) {
          if(guild_settings.unkowncmdmessage){
            message.channel.send(new Discord.MessageEmbed()
              .setColor(es.wrongcolor)
              .setFooter(es.footertext, es.footericon)
              .setTitle(`<a:no:921989165242003476> Unkown command, try: **\`${prefix}help\`**`)
        ...
        if (timestamps.has(message.author.id)) { //if the user is on cooldown
          ...
          if (now < expirationTime) { //if he is still on cooldonw
            ...
            return message.channel.send(new Discord.MessageEmbed()
              .setColor(es.wrongcolor)
              .setTitle(`<a:no:921989165242003476> Please wait ${timeLeft.toFixed(1)} more second(s) before reusing the \`${command.name}\` command.`)
      ...
      try{
        ...
        if(disablecmds.some(i => String(command.name).toLowerCase().includes(i.toLowerCase())) || 
          ...
          if(require('path').resolve(__dirname).split("/")[3].includes("FREEBOT_")){
            ...
            return message.channel.send(new MessageEmbed()
              ...
              .setTitle(`<a:no:921989165242003476> **THIS COMMAND IS PREMIUM ONLY** <a:nitro:833402717506502707>`)
        ...
        if(command.memberpermissions) {
          if (!message.member.hasPermission(command.memberpermissions)) {
            ...
            message.channel.send(new Discord.MessageEmbed()
              .setColor(es.wrongcolor)
              .setFooter(es.footertext, es.footericon)
              .setTitle("<a:no:921989165242003476> Error | You are not not_allowed to run this command!")
                ...
                //if not a DJ and not a nAdmin
                  ...
                  if(!player){
                    ...
                    not_allowed = true;
                      ...
                      .setTitle("<a:no:921989165242003476> Error | You are not allowed to run this command!")
            ...
            if(command.parameters.activeplayer){
              ...
              if (!mechannel){
                ...
                return message.channel.send(new MessageEmbed()
                  .setColor(ee.wrongcolor)
                  .setFooter(ee.footertext, ee.footericon)
                  .setTitle(`<a:no:921989165242003476> I am not connected to a Channel`)
                );
              }
            }
            //if no previoussong
            if(command.parameters.previoussong){
              if (!player.queue.previous || player.queue.previous === null){
                ...
                return message.channel.send(new MessageEmbed()
                  .setColor(ee.wrongcolor)
                  .setFooter(ee.footertext, ee.footericon)
                  .setTitle(`<a:no:921989165242003476> There is no previous song yet!`)
                );
              }
            }
        ...
        return message.channel.send(new Discord.MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(es.footertext, es.footericon)
          .setTitle("<a:no:921989165242003476> Something went wrong while, running the: `" + command.name + "` command")
   ...
   return message.channel.send(new MessageEmbed()
      .setColor("RED")
      .setTitle(`<a:no:921989165242003476> ERROR | An error occurred!`)
