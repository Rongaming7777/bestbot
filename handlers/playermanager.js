...
module.exports = async (client, message, args, type) => {
  ...
  if (!permissions.has("CONNECT"))
    return message.channel.send(new MessageEmbed()
      ...
      .setTitle("<a:no:921989165242003476> I need permissions to join your channel")
    );
  if (!permissions.has("SPEAK"))
    return message.channel.send(new MessageEmbed()
      .setColor(ee.wrongcolor)
      .setFooter(ee.footertext, ee.footericon)
      .setTitle("<a:no:921989165242003476> I need permissions to speak in your channel")
    );
  ...
  if (method[0] === "song")
    ...
    return message.channel.send(new MessageEmbed()
      .setColor(ee.wrongcolor)
      .setFooter(ee.footertext, ee.footericon)
      .setTitle("<a:no:921989165242003476> No valid search Term? ... Please Contact: `BROKARONAGAMING#9999`")
    );
}
