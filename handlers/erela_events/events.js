              ...
              client.channels.cache.get(player.textChannel).send(embed).then(msg => {
                ...
                collector.on(`collect`, async (reaction, user) => {
                  try {
                    ...
                    if (!channel) return message.channel.send(new MessageEmbed().setColor(es.wrongcolor).setFooter(es.footertext, es.footericon).setTitle(`<a:no:921989165242003476> You need to join a voice channel.`));
                    //get the lavalink erela.js player information
                    var player = client.manager.players.get(message.guild.id);
                    //if no player return
                    if(!player || player == undefined) return message.channel.send(new MessageEmbed().setColor(es.wrongcolor).setFooter(es.footertext, es.footericon).setTitle(`<a:no:921989165242003476> I am not playing anything in this Server`));
                    //if there is a player and the user is not in the same channel as the Bot return information message
                    if (player && channel.id !== player.voiceChannel) return message.channel.send(new MessageEmbed().setColor(es.wrongcolor).setFooter(es.footertext, es.footericon).setTitle(`<a:no:921989165242003476> I am already playing somewhere else!`).setDescription(`You can listen to me in: ${message.guild.channels.cache.get(player.VoiceChannel).name}`));
                    ...
                    switch (reactionemoji) {
                      ...
                      case String(emoji.react.skip_track):
                        ...
                        if (client.settings.get(message.guild.id, `djroles`).toString() !== "") {
                          ...
                          if (!player.get(`vote-${user.id}`)) {
                            ...
                            if (voteamount <= Number(player.get("votes"))) {
                              message.channel.send(new MessageEmbed()
                                .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                                .setFooter(es.footertext, es.footericon)
                                .setTitle(`<a:yes2:921988805442043925> Added your Vote!`)
                            ...
                            return message.channel.send(new MessageEmbed()
                              .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                              .setFooter(es.footertext, es.footericon)
                              .setTitle(`<a:yes2:921988805442043925> Removed your Vote!`)
                              .setDescription(`There are now: ${player.get("votes")} of ${voteamount} needed Votes`)
                            );
                          }
                        } else {
                          ...
                          if (player.queue.size == 0) {
                            ...
                            return message.channel.send(new MessageEmbed()
                              .setTitle(`<a:yes2:921988805442043925> ${emoji.msg.stop} Stopped and left your Channel`)
                              .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                              .setFooter(es.footertext, es.footericon)
                            );
                          }
                          ...
                          //send success message
                          return message.channel.send(new MessageEmbed()
                            .setTitle(`<a:yes2:921988805442043925> ${emoji.msg.skip_track} Skipped to the next Song`)
                            .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                            .setFooter(es.footertext, es.footericon)
                          );
                        }
                      ...
                      case String(emoji.react.autoplay_mode):
                        ...
                        //Send Success Message
                        message.channel.send(new MessageEmbed()
                          .setTitle(`<a:yes2:921988805442043925> ${player.get("autoplay") ? `${emoji.msg.enabled} Enabled` : `${emoji.msg.disabled} Disabled`} Autoplay`)
                          .setColor(es.color).setThumbnail(es.thumb ? es.footericon : null)
                          .setFooter(es.footertext, es.footericon)
                        ).then(msg => {
                          if(msg && msg.deletable) msg.delete({
