...
async function playtop(client, message, args, type) {
  ...
  async function song_() {
    ...
    if (!res.tracks[0]){
      return message.channel.send(`**<a:no:921989165242003476> Found nothing for: \`${search}\`**`);
    }
    //if track is too long return info msg
    if(res.tracks[0].duration > 3 * 60 * 60 * 1000){
      return message.channel.send(`**<a:no:921989165242003476> Cannot play a song that's longer than 3 hours**`)
    }
  ...
  async function playlist_() {
    ...
    for(const track of res.tracks)
      if(track.duration > 3 * 60 * 60 * 1000){
        return message.channel.send(`**<a:no:921989165242003476> Cannot play a song that's longer than 3 hours --> playlist skipped!**`)
      }
    let player;
    player = client.manager.create({
      guild: message.guild.id,
